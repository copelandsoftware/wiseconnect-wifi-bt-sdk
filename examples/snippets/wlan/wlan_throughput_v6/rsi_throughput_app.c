...
#define TCP_BUFF_SIZE  1440
#define UDP_BUFF_SIZE  1450
#define SSL_BUFF_SIZE  1200
...
#ifndef RSI_CONFIGURE_IPV6
//! Server IP address.
#define SERVER_IP_ADDRESS "192.168.1.116"
#else
...
int32_t application()
{
...
#else
  struct rsi_sockaddr_in6 server_addr, client_addr;
#endif
  ...
  switch (THROUGHPUT_TYPE) {
    case UDP_TX: {
      LOG_PRINT("\r\nUDP_TX TEST\r\n");
      // Create socket
      client_socket = rsi_socket(SOCKET_FAMILY, SOCK_DGRAM, 0);
      if (client_socket < 0) {
...
#else
      // Set server address family
      server_addr.sin6_family = SOCKET_FAMILY;
      // Set server port number, using htons function to use proper byte order
      server_addr.sin6_port = htons(SERVER_PORT);
      // Set IP address to localhost
      memcpy(server_addr.sin6_addr._S6_un._S6_u32, server_addr_6, RSI_IPV6_ADDRESS_LENGTH);
#endif
    ...
    case UDP_RX: {
      ...
      server_socket = rsi_socket_async(SOCKET_FAMILY, SOCK_DGRAM, 0, socket_async_recive);
#else
      server_socket = rsi_socket(SOCKET_FAMILY, SOCK_DGRAM, 0);
#endif
      if (server_socket < 0) {
        status = rsi_wlan_get_status();
        LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
    ...
    case TCP_TX: {
      LOG_PRINT("\r\nTCP_TX TEST\r\n");
      // Create socket
      client_socket = rsi_socket(SOCKET_FAMILY, SOCK_STREAM, 0);
      if (client_socket < 0) {
        status = rsi_wlan_get_status();
        LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
        return status;
...
#ifndef RSI_CONFIGURE_IPV6
      // Set family type
      client_addr.sin_family = SOCKET_FAMILY;
      // Set local port number
      client_addr.sin_port = htons(PORT_NUM);
#else
      // Set family type
      client_addr.sin6_family = SOCKET_FAMILY;
      // Set local port number
      client_addr.sin6_port = htons(PORT_NUM);
#endif
    ...
    case TCP_RX: {
      ...
      // Create socket
#if SOCKET_ASYNC_FEATURE
      server_socket = rsi_socket_async(SOCKET_FAMILY, SOCK_STREAM, 0, socket_async_recive);
#else
      server_socket = rsi_socket(SOCKET_FAMILY, SOCK_STREAM, 0);
#endif
      if (server_socket < 0) {
        status = rsi_wlan_get_status();
        LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
    ...
    case SSL_TX: {
      // Create socket
      client_socket = rsi_socket(SOCKET_FAMILY, SOCK_STREAM, 1);
      if (client_socket < 0) {
        status = rsi_wlan_get_status();
        LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
        return status;
...
#endif
      // Create socket
      client_socket = rsi_socket_async(SOCKET_FAMILY, SOCK_STREAM, 1, socket_async_recive);
      if (client_socket < 0) {
        status = rsi_wlan_get_status();
        LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%lX\r\n", status);
        return status;
